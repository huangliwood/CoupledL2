...
import coupledL3.HasCoupledL3Parameters
...
trait HasBOPParams extends HasCoupledL3Parameters {
...
class OffsetScoreTable(implicit p: Parameters) extends BOPModule {
  ...
  // On every eligible L3 read access (miss or prefetched hit), we test an offset d_i from the list.
