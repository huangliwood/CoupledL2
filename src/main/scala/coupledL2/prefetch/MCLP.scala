...
import coupledL2.prefetch.BestOffsetPrefetch
import coupledL2.prefetch.SPPParameters
import coupledL2.prefetch.SignaturePathPrefetch
...
trait HasMCLPParams extends HasCoupledL2Parameters {
...
abstract class PrefetchBranchV2Module(implicit val p: Parameters) extends Module with HasMCLPParams
abstract class PrefetchBranchV2Bundle(implicit val p: Parameters) extends Bundle with HasMCLPParams
...
class FilterV2(parentName:String = "Unknown")(implicit p: Parameters) extends PrefetchBranchV2Module with HasPerfLogging{
  ...
  val q = Module(new Queue(chiselTypeOf(UInt(fullAddressBits.W)), fTableQueueEntries, flow = true, pipe = false))
  ...
  val q_isFull = q.io.count === fTableQueueEntries.U
  q.io.deq.ready := q_isFull && q.io.enq.fire
...
class MCLPPrefetcher(parentName:String = "Unknown")(implicit p: Parameters) extends PrefetchBranchV2Module with HasPerfLogging{
  ...
  XSPerfAccumulate("prefetcher_has_evict", io.evict.valid)
}